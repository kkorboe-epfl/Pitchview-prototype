...
# python stitch_export_transform.py --left Footage16-11/20251116_102613_cam0.mp4 --right Footage16-11/20251116_102613_cam1.mp4 --preview --cylindrical --left-alpha 0.5 --save-calib rig_calibration.json
...
def save_calibration(path: str,
    ...
    H_list = np.asarray(H, dtype=float).tolist()  # plain Python floats
    off_list = [int(o) for o in offset]           # plain ints
    size_list = [int(s) for s in pano_size]       # plain ints
...
def load_calibration(path: str):
    """Load calibration from a JSON text file."""
...
def open_source(src: str, width: Optional[int], height: Optional[int]) -> cv2.VideoCapture:
    if is_int(src):
        ...
        if width: cam.set(cv2.CAP_PROP_FRAME_WIDTH, width)
        if height: cam.set(cv2.CAP_PROP_FRAME_HEIGHT, height)
...
def read_synced(capL, capR):
    okL, fL = capL.read()
    okR, fR = capR.read()
    if not okL or not okR: return False, None, None
    return True, fL, fR
...
def homography_is_sane(H: np.ndarray) -> bool:
    # Decompose roughly into scale/shear; reject if extreme
    ...
    # SVD to check anisotropy
    s = np.linalg.svd(lin, compute_uv=False)
    s_ratio = (s.max() / s.min()) if s.min() > 1e-12 else np.inf
    if s.max() > 5.0 or s.min() < 0.2:  # too much zoom in/out
        return False
    if s_ratio > 5.0:                   # too much shear
        return False
    # perspective terms not extreme
    if abs(H[2,0]) > 1e-3 or abs(H[2,1]) > 1e-3:
        # allow mild perspective but not huge
        return False
    return True
...
def compute_transform(firstL, firstR, use_cylindrical=False, downscale=0.5):
    # Downscale copies for robust feature matching
    def resize(img, s):
        if s == 1.0: return img
    ...
    H_ds, mask = cv2.findHomography(ptsR, ptsL, cv2.RANSAC, 3.0, maxIters=5000, confidence=0.995)
    use_affine = False
    if not homography_is_sane(H_ds):
        A, inl = cv2.estimateAffine2D(ptsR, ptsL, ransacReprojThreshold=3.0, maxIters=5000, confidence=0.995)
        if A is None:
    ...
    # Upscale H back to full-res coordinates
...
def feather_blend(base, overlay, mask_overlay, feather_px=40):
    ...
    w_b = 1.0 - w_o
    w_o3 = np.dstack([w_o]*3); w_b3 = np.dstack([w_b]*3)
...
def stitch_pair(frameL, frameR, H, offset, pano_size, left_alpha=1.0):
    ...
    T = np.array([[1,0,ox],[0,1,oy],[0,0,1]], dtype=np.float32)
    Hs = T @ H
    ...
    # clamp ROI to canvas (safety if offsets go OOB)
    ...
    if x1c > x0c and y1c > y0c:
        ...
        if 0.0 <= left_alpha < 1.0:
            ...
            cv2.addWeighted(roi_base, 1.0 - left_alpha, roi_left, left_alpha, 0.0, dst=roi_base)
        else:
            # opaque left
            roi_base[:] = roi_left
...
def main():
    ap = argparse.ArgumentParser(description="Robust panoramic stitch of two streams with single transform and sane canvas limits.")
    ...
    ap.add_argument("--downscale", type=float, default=0.5, help="Scale used only for transform estimation")
    ap.add_argument("--cylindrical", action="store_true", help="Apply cylindrical pre-warp before feature matching")
    ...
    ap.add_argument("--save-calib", type=str, default=None,
                    help="Path to save calibration (H, offset, pano size) as JSON")
    ap.add_argument("--load-calib", type=str, default=None,
                    help="Path to load calibration JSON and skip feature matching")
    args = ap.parse_args()
    ...
    if not ok:
        print("Could not read initial frames from both sources", file=sys.stderr); sys.exit(1)
    ...
    if firstL.shape[0] != firstR.shape[0]:
        ...
        def rh(img): 
            s = th / img.shape[0]
            return cv2.resize(img, (int(img.shape[1]*s), th), interpolation=cv2.INTER_AREA)
        firstL = rh(firstL); firstR = rh(firstR)

    # either load an existing calibration or compute a new one
    if args.load_calib:
        H, offset, pano_size, used_affine, meta = load_calibration(args.load_calib)
        print(f"[info] Loaded calibration from {args.load_calib}")
        # sanity check: warn if flags do not match the calibration file
        ...
        # compute transform (with safety)
        ds = max(0.2, min(1.0, args.downscale))
        H_raw, used_affine = compute_transform(firstL, firstR,
                                               use_cylindrical=args.cylindrical,
                                               downscale=ds)
        H, offset, pano_size = build_canvas(firstL, firstR, H_raw,
    ...
    print(f"[info] Transform: {'affine' if used_affine else 'homography'}  |  Panorama size: {pano_size}  |  Offset: {offset}")

    # prepare writer
    vw = writer_from_args(args.output, pano_size, args.fps) if args.output else None

    # preview window (scaled to fit)
    if args.preview:
    ...
    while True:
        ...
        if not ok: break
        # resize to match firstL height
        h_ref = firstL.shape[0]
        if fL.shape[0] != h_ref:
            s = h_ref / fL.shape[0]
            fL = cv2.resize(fL, (int(fL.shape[1]*s), h_ref), interpolation=cv2.INTER_AREA)
        if fR.shape[0] != h_ref:
            s = h_ref / fR.shape[0]
            fR = cv2.resize(fR, (int(fR.shape[1]*s), h_ref), interpolation=cv2.INTER_AREA)
        ...
        if frames % 10 == 0:
            ...
            cv2.putText(pano, f"{fps:.1f} fps", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255,255,255), 2, cv2.LINE_AA)

        if vw is not None: vw.write(pano)

        if args.preview:
            # cheap display scale to keep UI responsive
            scale = min(1.0, 1600 / max(pano.shape[1],1))
            if scale < 1.0:
                disp = cv2.resize(pano, (int(pano.shape[1]*scale), int(pano.shape[0]*scale)), interpolation=cv2.INTER_AREA)
            else:
                disp = pano
