...
# python stitch_two_streams.py --left left.mp4 --right right.mp4 --preview --cylindrical --left-alpha 0.5
...
def main():
    ...
    H, used_affine = compute_transform(firstL, firstR, use_cylindrical=args.cylindrical, downscale=max(0.2, min(1.0, args.downscale)))
    H, offset, pano_size = build_canvas(firstL, firstR, H, max_w=args.max-pano-w if hasattr(args,'max-pano-w') else args.max_pano_w, 
                                        max_h=args.max-pano-h if hasattr(args,'max-pano-h') else args.max_pano_h)
